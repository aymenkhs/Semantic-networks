...
class PropagationMarqueursException:
    ...
    def solve_for_node(self, node_id1, node_id2):
        ...
        while (len(open_direction1) != 0 or len(open_direction2) != 0) and not solution_found:
            try:
                ...
                open_direction1 += [conection.node for conection in node1.connections_from if conection.name == "is a" and connection.type != "exeption"]
            ...
            for node in open_direction1 + closed_direction1:
                for connection in node.connections_from + node.connections_to:
                    if connection.node in open_direction2 + closed_direction2 and connection.name != "is a":
                        solution_found = True
                        connecion_solution = connection

        return solution_found, connecion_solution
